defmodule Caju.Repositories.ContasCarteirasRepository do
  ...
  def reservar_saldo(carteira, amount) do
    ...
    query =
      from cc in ContasCarteiras,
        where:
          cc.id == ^carteira.id and cc.conta_id == ^carteira.conta.id and
  ...
  def lancar_transacao(carteira, amount, merchant) do
    amount_decimal = Decimal.new(to_string(amount))
      ...
      case Repo.update_all(query, []) do
        {count, _} when count > 0 ->
          extrato = %Extratos{
            ...
            id_conta: carteira.conta_id,
            id_carteira: carteira.carteira_id
          }
